...
def legal_score(text: str) -> tuple[int, list[str]]:
    score, reasons = _score(text, LEGAL_RULES + NEGATIVE_RULES)
...
def enrich(record: dict, extra_text: str = "") -> dict:
    ...
    legal, legal_reasons = legal_score(text)
    healthcare, healthcare_reasons = healthcare_score(text)
    return {
        **record,
        "legal_relevance_score": legal,
        "relevance_score": legal,
        "relevance": relevance_label(legal),
        "healthcare_relevance_score": healthcare,
        "sector": "Healthcare" if healthcare >= 70 else "Other",
        "practice_areas": classify_practice_areas(text),
        "score_reasons": legal_reasons,
        "healthcare_reasons": healthcare_reasons,
    }
